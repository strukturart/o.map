...
let select_gpx = function () {
  ...
  gpx_selection_info.distance = gpx_selection[gpx_selection_count]._info.length;
...
let update_tracking_view = () => {
  ...
  document.querySelector("#tracking-view .average-speed div").innerText = isNaN(
  ...
  document.querySelector("div#tracking-distance").innerText = tracking.distance;
};
...
const measure_distance = function (action) {
  ...
  if (action == "tracking") {
    ...
    tracking_interval = setInterval(function () {
      ...
      if (tracking_cache.length > 2) {
        ...
        localStorage.setItem("tracking_cache", JSON.stringify(tracking_cache));
  ...
  if (action == "addMarker") {
    ...
    l[l.length - 1]
      .bindPopup(calc.toString() + " " + general.measurement_unit, popup_option)
      .openPopup();
  }
};
