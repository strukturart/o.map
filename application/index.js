...
let mainmarker = {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    marker_text.addEventListener("blur", (event) => {
      ...
      if (c != "")
        mainmarker.selected_marker
          .bindPopup(c, module.popup_option)
          .openPopup();
    });
    ...
    function addMapLayers(action) {
      if (
        ...
        if (document.activeElement.hasAttribute("data-url")) {
          ...
          let item_maxzoom =
            document.activeElement.getAttribute("data-maxzoom");
    ...
    let coordinations = function () {
      ...
      //accurancy
      document.querySelector("div#coordinations div#accuracy span").innerText =
        mainmarker.accuracy.toFixed(2);
      ...
      if (mainmarker.current_alt) {
        document.querySelector("div#coordinations div#altitude").style.display =
          "block";
      ...
      if (mainmarker.current_heading != "unknown") {
        document.querySelector("div#coordinations div#heading").style.display =
          "block";
    ...
    function MovemMap(direction) {
      if (status.windowOpen == "map" || status.windowOpen == "coordinations") {
        ...
        if (direction == "left") {
          ...
          map.panTo(
            new L.LatLng(mainmarker.current_lat, mainmarker.current_lng)
          );
        }
    ...
    function shortpress_action(param) {
      switch (param.key) {
        ...
        case "Enter":
          ...
          if (
            document.activeElement == document.getElementById("clear-cache")
          ) {
            maps.delete_cache();
            break;
          }
          ...
          if (status.windowOpen == "marker") {
            document.querySelector("div#markers-option").style.display =
              "block";
